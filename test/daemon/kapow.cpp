...
int main(int argc, const char* argv[])
{
    ...
    std::unique_ptr<evhttp, decltype(&evhttp_free)> Server(
        evhttp_start(SrvAddress, SrvPort), &evhttp_free);
    if (!Server)
    {
    ...
    void (*OnReq)(evhttp_request * req, void*) = [](evhttp_request* req, void*) {
        ...
        if (!header_hash_str || !mix_hash_str || !nonce_str || !height_str || !share_boundary_str ||
            !block_boundary_str)
        {
            std::string error = "";
        ...
        else
        {
            ...
            evbuffer_add_printf(OutBuf, "{\"%s\":%s, \"%s\":%s, \"%s\":%s, \"%s\":\"%s\"}",
                "result", mix_match.c_str(), "share", share_met.c_str(), "block", block_met.c_str(),
                "digest", to_hex(result.final_hash).c_str());
